...
# 📄 Hardened Paper 2: The Digital Coherence Hypothesis (DCH)
...
Success is not defined by "correctness" alone, but by the probability P_{\text{local}} that the output could have been synthesized from local resources. We set the threshold for validation at five-sigma (\epsilon \approx 2.87 \times 10^{-7}).

This requires the target information to be sourced from a **Synthetic Logical Space (SLS)**—data that has never existed in the public domain and is computationally infeasible to "guess" (e.g., specific solutions to novel, high-complexity cryptographic or axiomatic problems).

---

## 4. Adversarial Reduction: Pre-empting "Clever Interpolation"

To harden the DCH against claims of "latent abstraction," we introduce the **Computational Burden Test**:
...
* **Confirmation Asymmetry:** A successful DCH result does not "prove" \Psi. It merely falsifies the specific locality-bound model of the CIS being tested, leaving the TFM as the most parsimonious surviving hypothesis.
